...
rawdata <- maml.mapInputPort(1) # class: data.frame
 ...
 ## v1: Credit score
  ...
  # summary(rawdata[[3]])
  # table(rawdata[[3]])
  rawdata[[3]][is.na(rawdata[[3]])] <- 1
  rawdata[[3]][rawdata[[3]] == "Y"] <- 1
  ...
  # summary(rawdata[[5]])
  # sum(rawdata[[5]] == "")
  rawdata[[5]][!is.na(rawdata[[5]])] <- 1
  rawdata[[5]][is.na(rawdata[[5]])] <- 0
  # table(rawdata[[5]])
  ...
  rawdata <- rawdata[rawdata[[18]] %in% c("CO","CP","LH","MH","PU","SF"), ]
